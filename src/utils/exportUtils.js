...
export function exportToExcel(households, pregnant, children, area = {}) {
  ...
  const hhRows = households.map((h) => [
    ...
    h.pregnantWomen,
    h.childUnder1Month,
    h.child1MonthTo1Year,
    h.child1To2Years,
    h.child2To5Years,
    h.child6To18Years,
    h.childMissedVaccine,
  ]);
  const totalRow = [
    "",
    "",
  ...
  ws1["!cols"] = [8, 22, 20, 25, 20, 14, 10, 10, 12, 12, 12, 12, 12, 14].map(
    (w) => ({ wch: w }),
  );
  ...
  ws2["!cols"] = [
    8, 20, 12, 6, 20, 14, 20, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12,
  ].map((w) => ({ wch: w }));
  ...
  const calcAgeMonths = (dob) => {
    ...
    const d = new Date(dob),
      t = new Date();
    return (
      (t.getFullYear() - d.getFullYear()) * 12 + t.getMonth() - d.getMonth()
    );
  };
...
export function exportToPDF(
  ...
  if (activeTab === "households" || activeTab === "all") {
    addHeader(
      ...
      `Total Houses: ${households.length}`,
    );
    addMeta(24);
    const body = households
      ...
      .map((h) => [
        h.id,
        h.headName + (h.guardianName ? ` (${h.guardianName})` : ""),
        h.address || "-",
        h.landmark || "-",
        h.contact,
        h.familyMembers,
  ...
  if (activeTab === "pregnant" || activeTab === "all") {
    ...
    addHeader(
      "Pregnant Women – Head Count Survey",
      `Total: ${pregnant.length}`,
    );
    addMeta(24);
    const calcAge = (dob) =>
      dob
        ? Math.floor((new Date() - new Date(dob)) / (365.25 * 86400000))
        : "-";
    autoTable(doc, {
      startY: 28,
      head: [
  ...
  if (activeTab === "children" || activeTab === "all") {
    if (activeTab === "all") doc.addPage();
    addHeader(
      "Children (0–2 Years) – Immunization Record",
  ...
  const filename =
    activeTab === "all" ? "Survey_Full_Report.pdf" : `Survey_${activeTab}.pdf`;
  doc.save(filename);
}
